...
const result1 = arr3.map(num => num * 10);
console.log(result1);
// Q3-2. filter함수와 arr3을 활용하여 [1,3,5]출력하기 (Hint:홀수와 짝수 구분은 "%2"를 활용)
const result2 = arr3.filter(num=> num%2==1); 
console.log(result2);
...
for (let i=1; i <= total1; i++) {
  let space = " ".repeat(total1 - i); // 앞에 들어갈 공백
  let star = "*".repeat(i);          // 출력할 별
  console.log(space + star);        // 공백 + 별 출력
}
...
for (let j = 1; j <= total2; j++) {
  ...
  // 공백 출력 (total - i)
  for (let k = 1; k <= total2 - j; k++) {
    line += " ";
  }

  // 별 출력 (2 * i - 1)
  for (let l = 1; l <= 2 * j - 1; l++) {
    line += "*";
  }
